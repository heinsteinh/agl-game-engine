...
namespace agl {
    ...
    public:
        ...
        // Drawing
        static void DrawArrays(const VertexArray& vao, const ShaderProgram& shader, GLenum mode = GL_TRIANGLES, uint32_t count = 0);
        static void DrawElements(const VertexArray& vao, const ShaderProgram& shader, GLenum mode = GL_TRIANGLES);

        // Textured drawing
        static void DrawElements(const VertexArray& vao, const ShaderProgram& shader, const Texture2D& texture, GLenum mode = GL_TRIANGLES);

        // Immediate mode helpers (for simple rendering)
        static void DrawQuad(const glm::mat4& transform, const glm::vec4& color);
        static void DrawTexturedQuad(const glm::mat4& transform, const Texture2D& texture, const glm::vec4& tint = glm::vec4(1.0f));
        static void DrawCube(const glm::mat4& transform, const glm::vec4& color);

        // Statistics
        static uint32_t GetDrawCallCount() { return s_stats.drawCalls; }
        static uint32_t GetVertexCount() { return s_stats.vertexCount; }
        static uint32_t GetTriangleCount() { return s_stats.triangleCount; }
