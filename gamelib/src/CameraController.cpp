...
CameraController::CameraController()
    : m_camera(nullptr), m_input(nullptr), m_mode(CameraMode::FirstPerson), m_settings(), m_target(0.0f, 0.0f, 0.0f),
      m_isAiming(false), m_isSprinting(false), m_isCrouching(false), m_inputEnabled(true), m_smoothingEnabled(true),
      m_velocitySmooth(0.0f), m_rotationSmooth(0.0f), m_targetFOV(75.0f), m_currentFOV(75.0f), m_shakeIntensity(0.0f),
      m_shakeDuration(0.0f), m_shakeTimer(0.0f), m_shakeOffset(0.0f), m_firstMouse(true), m_lastMouseX(0.0f),
      m_lastMouseY(0.0f) {
    AGL_CORE_INFO("CameraController created");
}

CameraController::CameraController(std::shared_ptr<Camera> camera) : CameraController() {
    SetCamera(camera);
}
...
void CameraController::ProcessKeyboard(float deltaTime) {
    ...
    if (glm::length(movement) > 0.0f) {
        ...
        if (m_isCrouching)
            speed *= m_settings.crouchMultiplier;
