...
Camera::Camera(glm::vec3 position, glm::vec3 up, float yaw, float pitch, float roll)
    : Position(position), WorldUp(up), Yaw(yaw), Pitch(pitch), Roll(roll), Front(glm::vec3(0.0f, 0.0f, -1.0f)),
      MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM), NearPlane(NEAR_PLANE), FarPlane(FAR_PLANE),
      AspectRatio(16.0f / 9.0f), OrthoLeft(-10.0f), OrthoRight(10.0f), OrthoBottom(-10.0f), OrthoTop(10.0f),
...
void Camera::SetPerspective(float fov, float aspectRatio, float nearPlane, float farPlane) {
    ...
    NearPlane = nearPlane;
    FarPlane = farPlane;
    AGL_CORE_TRACE("Camera set to perspective: FOV={:.1f}, Aspect={:.2f}, Near={:.2f}, Far={:.1f}", fov, aspectRatio,
                   nearPlane, farPlane);
}
...
void Camera::SetOrthographic(float left, float right, float bottom, float top, float nearPlane, float farPlane) {
    ...
    NearPlane = nearPlane;
    FarPlane = farPlane;
    AGL_CORE_TRACE("Camera set to orthographic: L={:.1f}, R={:.1f}, B={:.1f}, T={:.1f}", left, right, bottom, top);
}
...
bool Camera::IsInView(const glm::vec3 &point, float radius) const {
    ...
    return (ndcPos.x >= -1.0f - tolerance && ndcPos.x <= 1.0f + tolerance && ndcPos.y >= -1.0f - tolerance &&
            ndcPos.y <= 1.0f + tolerance && ndcPos.z >= -1.0f - tolerance && ndcPos.z <= 1.0f + tolerance);
}
